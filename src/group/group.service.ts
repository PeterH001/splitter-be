...
export class GroupService {
  ...
  async create(createGroupDto: CreateGroupDto, user: User) {    
    ...
    try {
      ...
      for (let i = 0; i < createGroupDto.userIds.length; i++) {
        for (let j = i + 1; j < createGroupDto.userIds.length; j++) {
          const userAid = createGroupDto.userIds[i];
          const userBid = createGroupDto.userIds[j];
