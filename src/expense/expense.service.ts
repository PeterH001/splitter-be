...
export class ExpenseService {
  ...
  async create(dto: CreateExpenseDTO) {
    try {
      ...
      expense.debts.forEach(async (debt) => {
        if (expense.payerId != debt.userId) {
          const balance = await this.prismaService.balance.findFirst({
            where: {
              groupId: expense.groupId,
              OR: [
                { userAId: expense.payerId, userBId: debt.userId },
                { userAId: debt.userId, userBId: expense.payerId },
              ],
            },
          });
          ...
          if(balance.userAId === debt.userId && balance.userBId === expense.payerId){
            await this.prismaService.balance.update({
              ...
              data:{
                userADebtsToB:{connect:{id: debt.id}}
              }
            })
          }//userB tartozik userA-nak
          else{
            await this.prismaService.balance.update({
              where:{
                id: balance.id
              ...
              data:{
                userBDebtsToA:{connect:{id: debt.id}}
              }
            })
          }
        }
  ...
  async update(id: number, dto: UpdateExpenseDTO) {
    try {
      ...
      for (let debtData of debtsData) {
        const oldDebt = expense.debts.find(
          (debt) => debt.userId === debtData.userId,
        );
        if (oldDebt != null) {
